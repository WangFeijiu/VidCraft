"""Whisper transcription worker — runs as a separate process."""
import sys
import json
...
def main():
    ...
    try:
        ...
        update_state(stage="processing", msg="语音转写中...", duration=dur,
                     transcribe_progress=[0, int(dur)])
        ...
        for s in segs_gen:
            if s.text.strip():
                sentences.append({"text": s.text.strip(),
                                  "start": round(s.start, 3),
                                  "end": round(s.end, 3)})
                if len(sentences) % 3 == 0:
                    update_state(stage="processing", msg="语音转写中...",
                                 duration=dur,
                                 transcribe_progress=[int(s.end), int(dur)])

        (project_dir / "sentences.json").write_text(
            json.dumps(sentences, ensure_ascii=False, indent=2), "utf-8"
        )
